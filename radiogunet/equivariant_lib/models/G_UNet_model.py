...
class G_RadioWNet (torch.nn.Module):
    def __init__ (self, config_first, model_path = './data/'):
        super (G_RadioWNet, self).__init__()
        self.first_GUNet = G_UNet(config_first)
        # self.second_GUNet = G_UNet (config_second)   <-- removed
        self.model_path = model_path
            
    def load_GUNet (self):
        print ('loading weights of first trained G-UNet')
        self.first_GUNet.load_state_dict(torch.load(
            os.path.join(self.model_path, 'best_first_model.mdl'),
            weights_only=True))
    
    # def load_second_GUNet (self):                   <-- entirely removed
    #     …

    def forward (self, x, stage='first'):
        # always run only the first U-Net
        return self.first_GUNet(x)
...
#         'kernel_size' : 3,
#         'padding'     : 1,
#         'equvariant_mask' : True,
#         'group'       : "D4"
#     }
    
#     G_WUNet = G_RadioWNet(config).to(device)
#     x = torch.rand(15,2,256,256).to(device)
#     y = G_WUNet(x)
#     print(y.shape)
